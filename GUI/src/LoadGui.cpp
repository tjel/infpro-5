...
#include <algorithm>
LoadGui::LoadGui(std::string FileName):mFileName(FileName)
{
    ...
    if(File->good())
    {
        LoadGUI();
        std::cout<<"Good\n";
        Date["Button"]["TextColorNormal"]->Good();//Przyklad
    }
}
void LoadGui::LoadGUI()
{
    std::string temp;
    ...
    while(!File->eof())//czeka na koniec pliku
    {
        ...
        if(temp.find("Part(",0) != std::string::npos && temp.find("Middle",0) != std::string::npos)
        {
            ...
            e = temp.find(':',0);
            f = temp.find(',',e);
            Date[ClassName][RemoveAllCharacter(temp.substr(0,e-1),' ')] = std::make_shared<ObjectGUI>(temp.substr(e+1,f-e-1),
        ...
        else if(temp.find("Middle",0) == std::string::npos && temp.find("Part",0) != std::string::npos)
        {
            ...
            e = temp.find(':',0);
            f = temp.find(',',e);
            Date[ClassName][RemoveAllCharacter(temp.substr(0,e-1),' ')] = std::make_shared<ObjectGUI>(temp.substr(e+1,f-e-1),IntRect::StringToRect(temp.substr(a,(b-a)+1)));
        }
        else if(temp.find("rgb",0) != std::string::npos)
        {
            ...
            d = temp.find(';',a);
            e = temp.find(':',0);
            Date[ClassName][RemoveAllCharacter(temp.substr(0,e-1),' ')] = std::make_shared<ObjectGUI>(Vector3i::StringToVector3(temp.substr(a,d-a)));
        }
...
std::string LoadGui::RemoveAllCharacter(std::string Date,char character)
{
